...
const descriptionText = `A total of <strong>$${totalRaised.toLocaleString()}</strong> has been raised for <strong>${totalGames}</strong> games. Currently, <strong>${unfundedGamesCount}</strong> ${unfundedGamesCount === 1 ? "game remains" : "games remain"} unfunded. ${
    unfundedGamesCount > 0 
        ? "We need your help funding these amazing games!" 
        : "Thank you for supporting all of our games!"
}`;

// create a new DOM element containing the template string and append it to the description container
const descriptionElement = document.createElement("p");
descriptionElement.classList.add("center-text");
...
topGameElement.innerHTML = `<strong>${topGame.name}</strong> ($${topGame.pledged.toLocaleString()} raised)`;
firstGameContainer.appendChild(topGameElement);

// do the same for the runner up item
const runnerUpElement = document.createElement("p");
runnerUpElement.innerHTML = `<strong>${runnerUp.name}</strong> ($${runnerUp.pledged.toLocaleString()} raised)`;
